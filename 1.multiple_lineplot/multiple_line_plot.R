...
data <- read.csv("./0135km9.csv", header = TRUE, row.names = 1)
for (i in sort(unique(data$cluster))){
  ...
  plot(avg, type = "l", ylim = c(min(tmp)*1.1, max(tmp)*1.1),ylab = "blabla xiao mo xian",
       xaxt="n", xlab = "", col=colours[2], lwd=2 )
  title(main = paste0("Quan shijie wo zui ke ai","_cluster_",i), cex.main = 1.2, font.main=1.5, line=1.5)
